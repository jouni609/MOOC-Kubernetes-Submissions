      ...
      <li class="flex items-center justify-between p-3.5 bg-slate-900/60 border border-slate-700/50 rounded-xl hover:border-slate-600 transition-colors group">
        ...
        <div class="flex items-center space-x-2 shrink-0">
          ...
          <button
            ...
            data-next-done="${todo.completed ? 'false' : 'true'}"
            class="text-[10px] font-semibold px-2.5 py-1 rounded-lg border ${todo.completed ? 'border-slate-600 text-slate-300 hover:bg-slate-700' : 'border-emerald-500/40 text-emerald-300 hover:bg-emerald-500/20'} transition-colors cursor-pointer"
          >
...
<body class="bg-slate-900 text-slate-100 min-h-screen flex flex-col items-center justify-center p-4">
  ...
  <script>
    document.addEventListener('click', async event => {
      const button = event.target.closest('[data-todo-id]')
      if (!button) return

      button.disabled = true

      try {
        const response = await fetch('/todos/' + encodeURIComponent(button.dataset.todoId), {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ done: button.dataset.nextDone === 'true' })
        })
