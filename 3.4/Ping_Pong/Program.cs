// Ping_Pong Application: Exercise 3.4
// Pong behavior lives at GET /. Cluster path /pingpong is rewritten by Gateway HTTPRoute.
...
// GET /health — load balancer health check (must not increment counter)
app.MapGet("/health", () =>
{
    Console.WriteLine("GET /health -> ok");
    return Results.Text("ok", "text/plain");
});

// GET / — pong counter (public /pingpong is rewritten here by HTTPRoute)
app.MapGet("/", async () =>
{
    try
    {
